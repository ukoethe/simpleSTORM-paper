...
image. Scalebar represents $1\mu m$}
...
\]
where $\mathrm{TP}$, $\mathrm{FN}$, and $\mathrm{FP}$ are the number of true positives, false negatives and false positives respectively, and $d(\vec{x}_i^*,\vec{x}_i)$ is the Euclidean distance of the $i^{\mathrm{th}}$ detection from the corresponding ground truth location. A true match is recorded if the ground truth position is within radius $r$ of the detection, such that each ground truth point can be assigned at most once. We use the evaluation tool from the ISBI challenge website to compute these metrics with $r=0.3 px$. 
...
Finally, we also applied SimpleSTORM to real data. A qualitative impression is given in figures \ref{fig:hela2-tubulin} and \ref{fig:hela2-actine}. In figure \ref{fig:hela2-tubulin}, we can even make a qualitative statement: the depicted microtubuli have a true diameter of about 25nm, whereas the diameter in the reconstruction is about 60nm. Considering the expected RMSE and the average distance of the fluorescent marker form the actual molecule, this is in the expected range.
\begin{figure*}
\begin{centering}
