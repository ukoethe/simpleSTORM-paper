...
parameters must be given. Usually, an isotropic Gaussian model like
(\ref{eq:gaussian-psf}) or an anisotropic version of it is sufficiently
accurate \citet{thompson_02_precise}. The free parameters of the
...
fitted model and the intensities of a spot candidate. 
...
$g(x,y)=g_{\mbox{PSF}}(x,y)+g_{n}(x,y)$. The signal and noise parts
...
according to Parseval's theorem. The signal-to-noise ratio at a point
...
over the response of many spots in order to get rid of the noise. 
...
least-squares optimization (Levenberg-Marquardt algorithm) such that
the squared difference between the model and the power spectrum is
minimized
\[
w_{1},w_{2},w_{3}=\mbox{argmin}_{w_{i}}\sum_{u,v}\left[w_{1}\,\exp\left(-\frac{r^{2}}{2w_{2}^{2}}\right)+w_{3}-\sqrt{P'(r)}\right]^{2}
\]
...
\]
where $W$ is the width of theROI used to compute the magnitude spectrum
...
define $\sigma_{\mbox{PSF}}$ as the median of the individual estimates.
Finally, the result of matched filtering is 
\[
\hat{s}_{t}=s_{t}\star\mbox{gauss}_{\sigma_{\mbox{PSF}}}
\]
where $\star$ denotes convolution.

Experiments reported in section ??? reveal that the parametric estimate
is usually superior to the non-parametric one, confirming that the
...
\]
where $\mbox{erfi}(.)$ is the inverse error function. That is, a
...
i.e. we filter with a Gaussian whose size $\sigma_{\mbox{PSF}}$ has
...
by the spline interpolator, see ???). Since the noise has unit variance
before filtering, the (unfiltered) signal-to-noise ratio is simply
given by  
\[
\mbox{SNR}=2g
\]
(the factor of 2 accounts for the smoothing effect of the matched
filter).  Maximal and minimal radii can be computed from the eigenvalues
...
\end{eqnarray*}
(these equations follow from basic properties of Gaussian filters)
...
maxima) when their centers have a distance of more than $2\sqrt{2}\sigma_{\mbox{PSF}}$.
...
To validate the PSFs width estimation using the Levenberg-Marquardt algorithm all PSFs from the first 2000 frames (all frames from high density data set) of the datasets provided from the ISBI challenge were detected and fitted. Figure \ref{histogram_of_sigmas} shows the distribution of the estimated width in histograms.
...
To give a objective impression of the SimpleSTORM algorithm the four trainingdata sets are processed with the standard parameters except the upscaling factor. Gain offset and PSF width were estimated from the data and the standard p-value of 0.0001 was used. For evaluation of the results the evaluation tool available on the ISBI challenge website was used. Tabels \ref{result_Training_0_1} to \ref{result_Training_1} show the results for Jaccard index, f-score, precision and RSME. For the evaluation only ground truth localisations within a lateral tolerance disk around the detected molecule positions are considered. Thus the radius of the lateral tolerance disk is crucial for the results. The radius was set at a rate of 0.1, 0.3, 0.5 and 1 to the pixelsize of the raw data. The RSME is given in nanometers.
\begin{table}
\caption{Radius 0.1}
...
\begin{tabular}{l|ccccc}
&Jaccard&F-Score&Precision&RSME\\ \hline
Tubulin1&0.445&0.615&0.937&15.6\\
Tubulin2&0.252&0.403&0.915&18.1\\
LS&0.770&0.870&0.949&9.80\\
HD&0.038&0.073&0.509&20.4
\end{tabular}
\label{result_Training_0_1}
\end{table}
