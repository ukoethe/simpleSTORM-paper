...
{\bf Finally, we applied SimpleSTORM to experimental single-molecule localization data recorded from cellular structures. The images we processed were recorded either using the principle of direct stochastic optical reconstruction microscopy (dSTORM,  \cite{heilemann_08_dstorm}, figures \ref{fig:microtubuli} and \ref{fig:hela2-tubulin}), or photoactivated-localization microscopy (PALM, \cite{betzig_06_imaging},  figure \ref{fig:hela2-actine}). All data were recorded with an exposure time of 100 ms and irradiation intensities between $1$ and $4\,\mathrm{kW/cm}^2$ (for experimental protocols, the reader is referred to \cite{heilemann_09_organic} (dSTORM, ATTO520), \cite{heilemann_08_dstorm} (dSTORM, Alexa Fluor 647) and \cite{muranyi_13_super-resolution} (PALM, mEos2)). Reconstructed super-resolution images are shown for tubulin labeled with ATTO520 (figure \ref{fig:microtubuli}) or Alexa Fluor 647 (figure \ref{fig:hela2-tubulin}), as well as for actin labeled with mEos2 (figure \ref{fig:hela2-actine}). We further estimate the spatial resolution by measuring the width of tubulin filaments, which were found to be about 60 nm (figure \ref{fig:microtubuli}) and comparable to previously published values (\cite{heilemann_08_dstorm}). Considering the expected RMSE and the average distance of the fluorescent markers from the actual molecule, this is in the expected ballpark.}
\begin{figure*}
\begin{centering}
...
Left: mean image of the raw data. Middle: reconstruction by SimpleSTORM. Right: magnification of the indicated subregion. Scalebars: 10~$\mu$m (first two images), 150~nm.}\label{fig:hela2-tubulin}
\end{figure*}
\begin{figure*}
\begin{centering}
