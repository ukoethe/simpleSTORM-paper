...
\par\end{centering}
\caption{Tubulin Example result from SimpleSTORM (data courtesy of Mike Heilemann).
Left: maximum projection of the raw data. Right: reconstructed high-resolution
image. Scalebar represents 1\,micro meter}
...
popular rapidSTORM software \citep{wolter_10_rapid-storm}, the ImageJ plugins Octane \citep{yu_11_octane}, PeakFit \citep{herbert_12_peakfit} and GraspJ (offering GPU accelaration, \cite{brede_12_graspj}), as well as the Localization Microscopy MicroManager plugin \citep{stuurman_12_plugin} and PYME (the Python localization microscopy environment, \cite{baddeley_12_pyme}).
...
$(x_{0},y_{0})$. Due to noise, we cannot simply detect the point with maximum intensity.
We assume that the noise is additive Gaussian noise with mean zero
...
\end{eqnarray}
The least-squares algorithm tries to determine the scaling $\mu_{m}$ and location $x_{m},y_{m}$ of a model $h(x,y)$ such that  the least-squares residual
\begin{multline}
\mu_{m}^{*},x_{m}^{*},y_{m}^{*}=\nonumber \\ 
...
last one has expected value zero (since the noise is uncorrelated),
...
\end{multline}
The maximization w.r.t. $\mu_{m}$ can be performed analytically
and yields
\[
\mu_{m}=\frac{\intRR f(x,y)h(x-x_{m},y-y_{m})\, d\vec{x}}{\intRR h(x-x_{m},y-y_{m})^{2}\,d\vec{x}}
\]
resulting in a simplified optimization problem for the location
\[
x_{m}^{*},y_{m}^{*}=\argmax_{x_{m},y_{m}}\frac{\left(\intRR f(x,y)h(x-x_{m},y-y_{m})\, d\vec{x}\right)^{2}}{\intRR h(x-x_{m},y-y_{m})^{2}\,d\vec{x}}
\]
Expanding the numerator by Cauchy's inequality gives
\begin{multline}
\frac{\left(\intRR f(x,y)h(x-x_{m},y-y_{m})\, d\vec{x}\right)^{2}}{\intRR h(x-x_{m},y-y_{m})^{2}\,d\vec{x}}\nonumber \\
\le\frac{\intRR f(x,y)^{2}\, d\vec{x}\intRR h(x-x_{m},y-y_{m})^{2}\, d\vec{x}}{\intRR h(x-x_{m},y-y_{m})^{2}\,d\vec{x}}
\end{multline}
and equality only holds when $h(x-x_{m},y-y_{m})=\kappa\, f(x,y)$.
Since $\kappa$ can be chosen arbitrarily, the maximum is achieved
for $x_{m}^{*}=x_{0}$ and $y_{m}^{*}=y_{0}$ as desired, and $h(x,y)$
must be the \emph{matched filter}
\[
h(x,y)=\psf(x,y)
\]
...
\end{equation}
assumes its maximum. This point can be conveniently determined by
...
a minor degradationat overlapping spots
...
whose distance from the line is at most a twentyth of the range between the minimal and the maximal value.
...
An inaccurate gain estimation due to missing beads could decrease the
performance of background estimation described later.
Therefore after the estimation of the gain and offset a post-optimization for the
gain is applied. The estimated parameters are used to transform the 
data. A histogramm of the intensities is created for the first 200 frames. 
Most pixels of these frames are background pixels which should be distributed
with unit variance around zero. A Gaussian is fitted to the histogram.
The gain factor is multiplied with the fitted variance and the variance of the
background pixels is estimated again. This is done until the fitted variance
is within a range of 0.05 around one.
...
now exhibits additive noise with unit variance and zero mean background
and thus conforms to the requirements of the matched
filter method. Figure \ref{fig:background-subtraction}
shows an example.

\begin{figure*}
\includegraphics[width=0.3\textwidth]{figures/Tubulin2OrigFrame50}$\qquad$\includegraphics[width=0.3\textwidth]{figures/dcframe4.png}$\qquad$\includegraphics[width=0.3\textwidth]{figures/lunet_raw_frame2547_rotated4.png}
...
\caption{\label{fig:background-subtraction}Upper row from left to right: original image (frame 50
of the Tubulin2 dataset), real dataset (actin conjugated to mEos2), real dataset (tubulin labeld with Cy5) .Lower row: the same frames after noise normalization and background subtraction according to equations (\ref{eq:anscombe-transform})
...
The three estimation methods perform similarly in experiments. This confirms that the Gaussian model is indeed applicable, because otherwise the Wiener filter would have been superior. Spatial domain fitting was slightly more robust than Fourier domain fitting, which occasionally failed to converge to the correct solution. Since Gaussian filtering is faster than Wiener filtering, we prefer the former in practice. Another advantage of the Gaussian filter is the possibility to change the filter strength, i.e. to use $\sigma_{\mathrm{filter}}\ne\sigma_{\psf}$. This is is useful for high density data with overlapping spots. Here, the matched filter may cause neighboring spots to be fused into one, leading to a detection error. In this case, it is possible to reduce filter strength to keep the spots separate at the price of slightly reduced localization accuracy of well-separated spots, cf. the experiment reported in section \ref{validatioin}.
...
every frame of the sequence. Spot detection is performed after standardization, i.e. after noise
...
by noticing that true spots always cover several pixels (according to the
...
Experiments reported in section \ref{bg_estimation} validate our claim that the false
positive rate (number of false positives over total number of background
pixels) is very low, namely 228 false positives out of 46 million for 
$p=1\%$ and 16 for $p=0.1\%$.
...
filter). The standard deviation of the 2D localization error $\Delta \bm{x}$ after matched filtering, defined in units of the pixel spacing, is then
\begin{equation}
\mathrm{StdDev}[\Delta \bm{x}]=\frac{1}{\sqrt{\pi}\,g}\label{eq:spot-std-dev}
\end{equation}
...
\[
\mathrm{StdDev}[\Delta \bm{x}']=\frac{\pi\,\sigma^2_{\psf}}{\sqrt{3}\,g'}
\]
...
\section{Results}
\subsection{Validation}\label{validatioin}
...
To validate the PSFs width estimation using the Levenberg-Marquardt algorithm all PSFs from the first 2000 frames (all frames from high density dataset) of the datasets provided from the ISBI challenge were detected and fitted. Figure \ref{histogram_of_sigmas} shows the distribution of the estimated width in histograms. The calculated values for the training datasets are 109~nm which is equivalent to 1.09 pixels for the high density and long sequence dataset and 0.73 pixels in the Tubulin datasets respectively. The estimated PSF width lies within a then percent range according to the calculated on. The only exception is the high density dataset for which the PSF width is overestimated because of the many overlapping PSFs.

\begin{figure}
\subfloat[Long sequence dataset, true sigma is 1.08]{\includegraphics[width = 0.24\textwidth]{figures/SigmaBundledTubes1.png}}\hfill%
\subfloat[High density dataset, true sigma is 1.08]{\includegraphics[width = 0.24\textwidth]{figures/SigmaBundledTubes2.png}}\hfill%
\subfloat[Tubulin 1 dataset, true sigma is 0.8]{\includegraphics[width = 0.24\textwidth]{figures/SigmaTubulin1.png}}\hfill%
\subfloat[Tubulin 2 dataset, true sigma is 0.8]{\includegraphics[width = 0.24\textwidth]{figures/SigmaTubulin2.png}}
...
\subsection{Background estimation} \label{bg_estimation}
...
To give an objective impression of the simpleSTORM algorithm the four training datasets are processed with the standard parameters except the upscaling factor. Gain offset and PSF width were estimated from the data and the standard p-value of 0.0001 (0.01~\%) was used. For evaluation of the results the evaluation tool available on the ISBI challenge website was used. Table \ref{result_Training_0_3} shows the results for Jaccard index, f-score, precision and RMSE. For the evaluation only ground truth localizations within a lateral tolerance disk around the detected molecule positions are considered. Thus the radius of the lateral tolerance disk is crucial for the results. The radius was set at a rate of 0.3 the pixel size of the raw data. The RMSE is given in nanometers.
%\begin{table}
%\caption{Radius 0.1}
%\begin{tabular}{l|ccccc}
%&Jaccard&F-Score&Precision&RMSE\\ \hline
...
Tubulin1&0.437&0.936&15.7&43\\
Tubulin2&0.232&0.904&18.8&52\\
LS&0.761&0.946&9.97&231\\
HD&0.031&0.449&20.55&50
\end{tabular}
...
\subsection{ISBI Localization Microscopy Challenge results}
The performance of the simpleSTORM algorithm was tested in the  ISBI Localization Microscopy Challenge. The results for different meassures like the f-score, the Jaccard index or the RMSE were calculated from the organizers of the challenge. The Jaccard index and the RMSE are contrary measures. Increasing the number of detected fluorophores by lowering the intensity threshold increases also the RMSE. This is because the number of fluorophores with lower intensity increases which can not be fitted as accurate as brighter fluorophores. Lowering the Jaccard index leads therefore to an decreasing RMSE. There were three test datasets used for the challenge. Each in a high density and a low density version. To present the results of the different softwares the rank for each software was calculated for each measure and dataset individually. After that the mean rank on both, the Jaccard index (higher is better and therefore yields a lower rank) and the RMSE (lower is better) for each dataset was calculated. Figure \ref{scatterplot_challenge} shows the results for the high density datasets and the low density datasets. Software that achieved high Jaccard indices are present in the lower part of the scatter plot, software with small RMSE can be found on the left. The nearer an algorithm is located to the origin, the better it performs. SimpleSTORM is closest to the origin for the high density datasets and within the best 5 algorithms for the low density datasets. 

Figure \ref{tubulin1} and Figure \ref{highdensity} show the average projection of the raw data, the reconstruction and a smaller part of the image with the underlying groundtruth of both the tubulin 1 training dataset and the high density dataset from the ISBI Localization Microscopy Challenge. 


\begin{figure}
\subfloat[High density datasets]{\includegraphics[width = 0.49\textwidth]{figures/high_Density_scatter.pdf}}\hfill%
\subfloat[Low density datasets]{\includegraphics[width = 0.49\textwidth]{figures/low_Density_scatter.pdf}}\hfill%
	\caption{Histograms for three training datasets from the ISBI localization challenge}%
...
%\begin{figure*}
%\begin{centering}
%\includegraphics[width=0.45\textwidth]{figures/Bildmike_projection}$\qquad$\includegraphics[width=0.45\textwidth]{figures/Bildmike_reconstruction.png}
%\par\end{centering}
...
\caption{Tubulin 1 dataset from the ISBI Localization Microscopy Challenge. Left: mean projection of the raw data. Middle: reconstructed image. Right: localizations (green) and groundtruth (red). Scalebar represents 2\,micrometer for the first two images and 100\,nm for the third.}\label{tubulin1}
\end{figure*}

\begin{figure*}
\begin{centering}
\includegraphics[width=0.3\textwidth]{figures/Highdensity_raw_scalebar.png}$\qquad$\includegraphics[width=0.3\textwidth]{figures/highdensityreconstruction_rect_hc_scalebar.png}$\qquad$\includegraphics[width=0.3\textwidth]{figures/highdensityAusschnitt_gt_scalebar.png}
\par\end{centering}

\caption{High density dataset from the ISBI Localization Microscopy Challenge. Left: mean projection of the raw data. Middle: reconstructed image. Right: localizations (green) and groundtruth (red). Scalebar represents 1\,micrometer for the first two images and 200\,nm for the third.}\label{highdensity}
\end{figure*}
...
\caption{Beta-tubulin labeld with AlexaFluor647 in a Hela cell. Example result from simpleSTORM.
Left: one frame of the raw data. Middle: reconstructed high-resolution
image. Right: magnified, selected part from the reconstructed image.}
\end{figure*}
...
Left: one frame of the raw data. Middle: reconstructed high-resolution
image. Right: magnified, selected part from the reconstructed image. Scalebar represents 3\,micrometer}
\end{figure*}
